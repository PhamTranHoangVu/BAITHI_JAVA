		...
		musicplay.getOnCompletionListener(new OnCompletionListener() {
			...
			public void OnEndMusic() {
				...
				if(result==0)
				{
					...
					Toast.makeText(getApplicationContext(),"đã tắt chế độ phát ngẫu nhiên", Toast.LENGTH_SHORT).show();
				}
				//
				if(result_Repeat==1)
				{
	...
	private class Click implements OnClickListener
	{
		...
		public void onClick(View arg0) {
			...
			case R.id.iv_shuffle:
				...
				if(result==1 && KT==0)
				{
					Toast.makeText(getApplicationContext(),"đã bật chế độ phát ngẫu nhiên", Toast.LENGTH_SHORT).show();
				}
			...
			case R.id.iv_previous:
				...
				break;
			case R.id.iv_repeat:
				if(musicplay.getState() == com.example.music.MusicPlay.PLAYER_PLAY && KT_Next_Repeat==0)
				{
					...
					KT_Next=0;
					result_Repeat =1;
					Toast.makeText(getApplicationContext(),"đã bật chế độ lặp lại bài hát", Toast.LENGTH_SHORT).show();
				}
				if(KT_Next_Repeat == 1 && musicplay.getState() == com.example.music.MusicPlay.PLAYER_PLAY )
				{
					...
					KT_Next_Repeat =0;
					result=0;
					Toast.makeText(getApplicationContext(),"đã tắt chế độ lặp lại bài hát", Toast.LENGTH_SHORT).show();
				}
				break;
			default:
				break;
